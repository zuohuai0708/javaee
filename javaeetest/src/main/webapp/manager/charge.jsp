...
function charge(test){
	...
	var sign = hex_md5(order+""+chargekey);
...
<div  style="background: none repeat scroll 0 0 #FFFFFF; margin:10px; float:left; padding:10px; width:450px; height:300px; border: 1px solid #D8D8D8;">
	<table>
		...
		<tr><td>金币：</td><td><input type="text" id="gold" name="gold" value="0" onkeyup="onChangeGold(this.value);" /></td></tr>
		<tr><td>金额：</td><td><input type="text" id="money" name="money" value="0" /></td></tr>
		<tr><td>订单号：</td><td><input type="text" id="order" name="order"/>&nbsp;
								<a href="javascript:;" onclick="changeOrder();return false;">更换订单号</a></td></tr>
		<tr><td>附加信息：</td><td><input type="text" id="addition" name="addition"/></td></tr>
		<tr><td></td><td><button onclick="charge(false);">充值</button><button onclick="charge(true);">测试</button>&nbsp;&nbsp;<button onclick="window.location='../'">返回</button></td></tr>
	</table>
</div>
</body>
</html>